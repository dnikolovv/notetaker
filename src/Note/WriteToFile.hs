...
writeNote :: MonadIO m => Note -> FilePath -> m FilePath
writeNote note path = do
  file <- liftIO . getNonconflictingPathForFile $ path
  liftIO $ TextIO.writeFile file . noteContents $ note
  return file

getNonconflictingPathForFile :: MonadIO m => FilePath -> m FilePath
getNonconflictingPathForFile path = do
  let basename = dropExtension path
      extension = takeExtension path
  liftIO $ findGoodPath basename extension 0
  where
    findGoodPath b e i =
